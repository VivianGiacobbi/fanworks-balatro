...
extern MY_HIGHP_OR_MEDIUMP vec4 r_replace = vec4(1.0, 1.0, 1.0, 1.0);
extern MY_HIGHP_OR_MEDIUMP vec4 g_replace = vec4(0.0, 0.0, 0.0, 1.0);
extern MY_HIGHP_OR_MEDIUMP vec4 b_replace = vec4(0.0, 0.0, 0.0, 0.0);
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    if (tex.r >= 1. && tex.g <= 0. && tex.b <= 0) {
        tex = r_replace;
    } else if (tex.r <= 0. && tex.g >= 1. && tex.b <= 0.) {
        tex = g_replace;
    } else if (tex.r <= 0. && tex.g <= 0. && tex.b >= 1.) {
        tex = b_replace;
    }
