...
extern MY_HIGHP_OR_MEDIUMP number vertex_scale_mod = 1.0;
extern MY_HIGHP_OR_MEDIUMP float mask_offset = 0.5;
extern MY_HIGHP_OR_MEDIUMP float wave_time;
extern MY_HIGHP_OR_MEDIUMP number wave_t;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    MY_HIGHP_OR_MEDIUMP vec2 sin_coords = vec2(texture_coords.x + 0.01 * sin((wave_t * PI * texture_coords.y + (30.0*PI/180.0)) + wave_time), texture_coords.y);
    
    MY_HIGHP_OR_MEDIUMP vec4 tex = vec4(0.0);
    vec4 mask = Texel(texture, texture_coords + vec2(mask_offset, 0.0)); // mask colour
    if (mask.r > 0.0) {
       tex = Texel(texture, sin_coords);
    }
    ...
    return dissolve_mask(tex, sin_coords, uv);
}
