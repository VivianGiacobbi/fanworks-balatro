...
#define MASK_OFFSET vec2(0.6, 0.0) // mask frame is 0.4 units away from the soul frame (normalised)
#define BASE_OFFSET vec2(0.0, 0.0) // base frame is -0.2 units away from the soul frame (normalised)
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    vec2 scaled_texture_coords = (texture_coords - centre) / 1 + centre; // scale the entire uv to counter edge clipping
    ...
    if (mask.r > 0.0 && mask.g > 0.0) {
        base_color = vec4(hue_shift(base_color.rgb, hue_mod), base_color.a);
    }
    
    // required for dissolve fx
    return dissolve_mask(base_color*colour, scaled_texture_coords, dissolve_uv);
}
